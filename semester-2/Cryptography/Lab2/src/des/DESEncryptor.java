...
public class DESEncryptor {
    ...
    public byte[] encrypt(byte[] message, byte[] key) {
        ...
        int i = 0;
        int k = 0;
        byte[] feedback = getInitializationVectorCFB();
    ...
    public byte[] decrypt(byte[] message, byte[] key) {
        if (message.length < 8) {
            System.out.println("Message should be at least 64 bits");
            System.exit(1);
        }
        if (key.length != 8) {
            System.out.println("Key should be 64 bits");
    ...
    public byte[] encryptText(byte[] message, byte[] key) {
        if (message.length != 8) {
        ...
        for (byte[] subKey : subKeys) {
            ...
            /* XOR rightHalf with subKey */
            rightHalf = performXOR(rightHalf, subKey);
    ...
    private byte[] sBox(byte[] input) {
        ...
        for (int i = 0; i < input.length; i++) {
             ...
             */
            int row = (block & 0x0001) + (block >> 5 & 0x0001);
            int col = block >> 1 & 0x000F;

            int[] selectedSBox = getSBox(i);
            output[i] = (byte) selectedSBox[16 * row + col];
        }
        return output;
    }
    ...
    public static void main(String[] args) {
        try {
            ...
            byte[] key = des.hexStringToByteArray(keyString);
...
//    public static void main(String[] args) {
//        try {
//            des.DESEncryptor des = new des.DESEncryptor();
//            File keyFile = new File("DESkey.txt");
//            File textFile = new File("DESplaintext.txt");
//            File cipherFile = new File("DESciphertext.txt");
//            FileReader keyFileReader = new FileReader(keyFile);
...
//            byte[] message = des.decrypt(cipher, key);
